...
export class OceanOSService {
  ...
  constructor(private dts: DateTimeService, private http: HttpClient) {
    combineLatest([
      ...
      this.http.get<User>(`/api/users/oos`),
    ]).subscribe(([ss, ds, us]) => {
      this.streamType = new Map(
        ss.filter((s) => s.type.endsWith("TYPE")).map((s) => [s.type, s])
      );
  ...
  putDrop(d: IDrop): Promise<Object> {
    const drop = new Drop({
      ...d,
      uid: this.user._id,
      color: this.previewColor(d, this.startAt)
    });
    if (drop._id === "new") {
  ...
  unselectedStreams() {
    return this.streams$.pipe(
      map((tags) => tags.filter((t) => !t.selected && !t._id.endsWith("_TYPE")))
    );
  }
  ...
  private previewColor(drop: IDrop, startAt: number): string {
    ...
    const color = this.dts.isWithin(drop.date, {
      start: startAt,
      end: this.dts.addDay(startAt),
    })
      ? "day"
      : this.dts.isWithin(drop.date, {
          start: startAt,
          end: this.dts.addWeek(startAt),
        })
      ? "week"
      : this.dts.isWithin(drop.date, {
          start: startAt,
          end: this.dts.addMonth(startAt),
        })
      ? "month"
      : this.dts.isWithin(drop.date, {
          start: startAt,
          end: this.dts.addYear(startAt),
        })
      ? "year"
      : "";
  ...
  fromTime(stream: HomeStream) {
    ...
    this.previewAt =
      stream.preview === "day"
        ? this.dts.addDay(stream.startAt)
        : stream.preview === "week"
        ? this.dts.addWeek(stream.startAt)
        : stream.preview === "month"
        ? this.dts.addMonth(stream.startAt)
        : stream.preview === "year"
        ? this.dts.addYear(stream.startAt)
        : stream.startAt;

    for (let d of this.dropsMap.values()) {
      d.color = this.previewColor(d, stream.startAt);
      d.available =
        (d.date < this.previewAt && !d.available) ||
        (!(d.date > this.previewAt) && d.available);
    }
    this.filterStreamDrops();
  }
  ...
  getPublicStream(uid: string, streams: string[]): Observable<IDrop[]> {
    return this.http.post<IDrop[]>(`/api/users/stream/`, {
      uid: uid,
      streams: streams,
    });
  }
  ...
  getStreamById(
    ...
    return this.http.get<Stream>(
      `/api/streams/urn:oos:0x0:${userid}:stream:${sname}:PUBLIC`
    );
  }
  ...
  private filterStreamDrops() {
    ...
    if (fstreams.length !== 0) {
      ...
      const availableDrops: Map<string, Drop> = new Map(
        [...this.dropsMap.values()]
          .filter(
            // for every filter tag, the drop must contain some
            (d: IDrop) =>
              d.date < this.previewAt &&
              fstreams.every((fts: Stream) =>
                d.streams.some((s: IStream) => s._id === fts._id)
              )
          )
          .map((d) => [d._id, d])
      );
      ...
      const availableStreams = new Map(
        [...availableDrops.values()].flatMap((d) =>
          d.streams.map((s) => [s._id, s])
        )
      );

      // update available drops
      for (let d of this.dropsMap.values())
        d.available = !!availableDrops.get(d._id);

      // update available tags
      for (let s of this.streamsMap.values())
        s.available = !!availableStreams.get(s._id) && !s.filtered;
    } else {
      ...
      for (let d of this.dropsMap.values())
        d.available = d.date < this.previewAt;
    }
    this.newStreams();
    this.newDrops();
  }
